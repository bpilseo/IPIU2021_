...
from config import CFP_DATA_DIR
import argparse
import tqdm

def parseList(root):
    with open(os.path.join(root, 'cfp-ff-pair.txt')) as f: # cfp-fp-pair.txt
...
def getFeatureFromTorch(cfp_dir, feature_save_dir, resume=None, gpu=True):
    ...
    net.eval()
    nl, nr, flods, flags = parseList(cfp_dir)
    cfp_dataset = CFP(nl, nr)
    cfp_loader = torch.utils.data.DataLoader(cfp_dataset, batch_size=32,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfp_dir', type=str, default=CFP_DATA_DIR, help='The path of cfp data')
    ...
    getFeatureFromTorch(args.cfp_dir, args.feature_save_dir, args.resume)
