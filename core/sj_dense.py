...
class MobileFacenet(nn.Module):
    def __init__(self, bottleneck_setting=Mobilefacenet_bottleneck_setting):
        ...
        self.conv2 = ConvBlock(256, 512, 1, 1, 0)
    ...
    def _make_layer(self, block, setting):
        ...
        [2, 128, 2, 1]
        """
        for e, c, n, s in setting:
            flag = 0
            for i in range(n):
                if i == 0:
                    layers.append(block(self.inplanes, c, s, e))
                else:
                    flag =1
                    layers.append(block(self.inplanes, c, 1, e))

                if flag and self.inplanes == c:
                    self.inplanes = c*2
                    flag = 0
                else:
                    self.inplanes = c
